...
const CrearPedidoPage = () => {
    ...
    const handleDetalleChange = <K extends keyof DetallePedido>(
        ...
        if (field === "id_producto" || field === "cantidad_pedida") {
            const producto = productos.find(p => p.id_producto === nuevosDetalles[index].id_producto);
            const cantidad = Number(nuevosDetalles[index].cantidad_pedida);
            nuevosDetalles[index].precio_total = producto ? producto.precio_unitario * cantidad : 0;
        }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!form.id_construccion || detalles.length === 0) {
            alert("Debes seleccionar una construcción y agregar al menos un producto.");
            return;
        }

        const camposInvalidos = detalles.some(detalle =>
            !detalle.id_producto || detalle.cantidad_pedida <= 0 || !detalle.fecha_estimada_entrega
        );

        if (camposInvalidos) {
            alert("Todos los productos deben tener valores válidos (producto, cantidad, fecha estimada).");
            return;
        }
                    ...
                    {detalles.map((detalle, index) => (
                        <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                            ...
                            <div className="flex items-center gap-2">
                                ...
                                <button type="button" onClick={() => eliminarDetalle(index)} className="text-red-600 text-sm hover:underline">
                                    Eliminar
                                </button>
                            </div>
                        </div>
